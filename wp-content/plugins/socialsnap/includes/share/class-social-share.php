...
class SocialSnap_Social_Share {
	...
	public function init() {
		...
		if ( isset( $_GET['ss_network_authorized'], $_GET['access_token'], $_GET['expires_in'] ) && 'facebook_shares' === $_GET['ss_network_authorized'] ) { // phpcs:ignore
					...
					<div class="ss-popup-networks ss-clearfix">
						...
						foreach ( $all_networks as $id => $name ) {
							...
							<div class="ss-popup-network ss-popup-<?php echo esc_attr( $id ); ?>">
								<a href="#" data-ss-ss-link="<?php echo esc_url( $share_url, $allowed_protocols ); ?>" data-id="<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $icon_class ); ?>" <?php echo esc_html( $additional_data ); ?> rel="nofollow noopener">
			...
			foreach ( $data['options']['networks'] as $network => $network_settings ) {
				...
				<li class="<?php echo esc_attr( $hide_class ); ?>">
					...
					<?php if ( 'on_media' === $location ) { ?>
						...
						<div data-ss-ss-link="<?php echo esc_url( $share_url ); ?>" class="<?php echo esc_attr( $icon_class ); ?>" <?php echo esc_html( $additional_data ); ?>>
					<?php } else { ?>

						<?php $icon_class = apply_filters( 'socialsnap_social_share_button_class', 'ss-' . $network . '-color', $network, $data['post_id'] ); ?>

						<a href="#" data-ss-ss-link="<?php echo esc_url( $share_url ); ?>" class="<?php echo esc_attr( $icon_class ); ?>" rel="nofollow noopener" <?php echo esc_html( $additional_data ); ?>>
					<?php } ?>
	...
	public function add_share_button_api_data( $data, $url, $permalink, $network, $post_id, $location ) {
		...
		$data .= ' data-ss-ss-network-id="' . esc_attr( $network ) . '"';
		$data .= ' data-ss-ss-post-id="' . esc_attr( $post_id ) . '"';
		$data .= ' data-ss-ss-location="' . esc_attr( $location ) . '"';
		$data .= ' data-ss-ss-permalink="' . esc_url( $permalink ) . '"';
	...
	public function add_settings_config( $settings ) {
		...
		if ( false !== $facebook_token ) {
			...
			if ( 'never' !== $facebook_token['expires_in'] && time() < intval( $facebook_token['expires_in'] ) ) {
				...
				$note['ss_ss_facebook_authorize_note']['desc'] = __( 'Expires on ', 'socialsnap' ) . gmdate( 'F j, Y', $facebook_token['expires_in'] ) . '.';
